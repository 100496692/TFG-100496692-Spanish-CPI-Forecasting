...
print("="*100)
print("RANDOM FOREST - 3 SCENARIO VALIDATION")
print("="*100)
...
for sc in scenarios:
    ...
    for version, features in [('A_Food', features_A), ('B_AllExog', features_B)]:
        ...
        print(f"    RF_{version}: MAE={mae:.4f} | RMSE={rmse:.4f}")

        scenario_preds[sc['label']][f'RF_{version}'] = pred
        all_results.append({'Scenario': sc['name'], 'Label': sc['label'],
                            'Model': f'RF_{version}', 'MAE': mae, 'RMSE': rmse})
...
rf_final.fit(train_full[features_B], train_full[target])
...
ax.invert_yaxis()
plt.tight_layout()
plt.savefig('figure_5_rf_feature_importance.png', dpi=300, bbox_inches='tight')
print("  ✓ Saved: figure_5_rf_feature_importance.png")
plt.close()
...
for idx, sc in enumerate(scenarios):
    ...
    mae_a = [r['MAE'] for r in all_results if r['Label']==label and r['Model']=='RF_A_Food'][0]
    mae_b = [r['MAE'] for r in all_results if r['Label']==label and r['Model']=='RF_B_AllExog'][0]

    ax.plot(dates, actual.values,           color='black',   linewidth=2.5, marker='o', markersize=4, label='Actual', zorder=5)
    ax.plot(dates, preds['RF_A_Food'],      color='#F77F00', linewidth=2,   linestyle='--', label=f'RF+Food (MAE:{mae_a:.4f})')
    ax.plot(dates, preds['RF_B_AllExog'],   color='#9D4EDD', linewidth=2,   linestyle='--', label=f'RF+AllExog (MAE:{mae_b:.4f})')
...
plt.suptitle('Random Forest — Multi-Scenario Validation', fontsize=14, fontweight='bold', y=0.995)
plt.tight_layout()
plt.savefig('figure_5_rf_multiscenario.png', dpi=300, bbox_inches='tight')
print("  ✓ Saved: figure_5_rf_multiscenario.png")
plt.close()

# Save
results_df = pd.DataFrame(all_results)
results_df.to_csv('rf_results_3scenarios.csv', index=False)
imp_df.to_csv('rf_feature_importance_3scenarios.csv', index=False)
